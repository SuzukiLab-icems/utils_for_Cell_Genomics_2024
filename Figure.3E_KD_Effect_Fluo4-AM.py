...
for values in factors:
	...
	for target in ["Ovol2","Rnf215","Cldn34c4","Rd3","Cebpg","Rbm26","P2ry2"]:
		...
		try:
			...
			with open("./Figure.3E_KD_Effect_Fluo4-AM/Figure.3E_KD_Effect_stats.txt", "a", encoding='utf-8', newline='\n') as f:
				f.write(f'({values}-{target}):mannwhitney={u},ttest={t} \n')
		except TypeError:
			print("not appropriate")
	sns.set(style="ticks", font_scale=2.5, font='arial',
		rc = {'figure.figsize':(18,12), 'axes.linewidth':1.5})
	...
	plt.xlabel("shRNA", fontsize=font_size, rotation=0)
	...
	plt.tight_layout()
	plt.savefig(f'./Figure.3E_KD_Effect_Fluo4-AM/Figure.3E_KD_{values}.eps', dpi=300)
	plt.savefig(f'./Figure.3E_KD_Effect_Fluo4-AM/Figure.3E_KD_{values}.png', dpi=300)
	plt.close()
