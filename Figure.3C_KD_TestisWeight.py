...
for values in factors:
	...
	for target in ["Ovol2","Rnf215","Cldn34c4","Rd3","Cebpg","Rbm26","P2ry2"]:
		...
		try:
			...
			with open("./Figure.3C_KD_TestisWeight/Figure.3C_KD_TestisWeight_Stats.txt", "a", encoding='utf-8', newline='\n') as f:
				f.write(f'({values}-{target}):mannwhitney={u},ttest={t} \n')
		except TypeError:
			print("not appropriate")
	sns.set(style="ticks", font_scale=2.5, font='arial',
		rc = {'figure.figsize':(16,10), 'axes.linewidth':1.5})
	...
	plt.tight_layout()
	plt.savefig(f'./Figure.3C_KD_TestisWeight/Figure.3C_{values}.eps', dpi=300)
	plt.savefig(f'./Figure.3C_KD_TestisWeight/Figure.3C_{values}.png', dpi=300)
	plt.close()
